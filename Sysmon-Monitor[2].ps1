...
- Sysmon-Monitor[2].ps1 changes: adds classification layer before popup, targeted evidence-only/self-noise handling, sysmontoast-only toast launch route, and selective Autoruns gating.
#>
...
$AlertPath = "C:\Users\RASPUTIN4149\AppData\Local\Temp\Sysmon\sysmon_alert_state.txt"
...
function Send-SysmonToast {
    ...
    # FIX 1: Explicitly define the path inside the function scope
    
    $AppId = "Microsoft.WindowsTerminal_8wekyb3d8bbwe!App" 
    
    $ToastXml = [Windows.Data.Xml.Dom.XmlDocument]::New()
     # === Properly escaped launch command ===
    ...
    if ($DisplayPopup -eq $true ){
		$ToastXmlText = @"
<toast launch='sysmontoast://popup-set'>
		<visual>
		...
		<actions>
			<action content="Hide AlertPopup"
					arguments="sysmontoast://alert-unset"
...
<toast launch='sysmontoast://popup-set'>
		...
		<actions>
			<action content="Display AlertPopup"
					arguments="sysmontoast://alert-set"
					activationType="protocol" />
...
"@
    ...
    try {
		...
		Write-DebugLine "$(Get-LineNumber)Toast XML was: $ToastXml"
...
function Show-SysmonAlert {
    ...
    try {
        $AlertReadDeadline = (Get-Date).AddSeconds(4)
        ...
        do {
            if (Test-Path -Path $AlertPath) {
                ...
                if ($AlertControl -eq "SET" -or $AlertControl -eq "UNSET") {
                    break
                }
            }
            else {
                Write-DebugLine "$(Get-LineNumber)Alert control file does not exist yet: $AlertPath"
            }

            Start-Sleep -Milliseconds 500
        }
        while ((Get-Date) -lt $AlertReadDeadline)
    }
    catch {
    ...
    $ClassificationSummary = "N/A"
    try {
        $ClassValues = @($Report | ForEach-Object {
            if ($_.PSObject.Properties.Name -contains "AlertClass") {
                [string]$_.AlertClass
            }
    ...
    Write-DebugLine "$(Get-LineNumber)Show-SysmonAlert decision: DisplayPopup=$DisplayPopup AlertControl=[$AlertControl] ShouldDisplayPopup=$ShouldDisplayPopup ToAnalyzeFolder=[$ToAnalyzeFolderName] Classification=[$ClassificationSummary]"

    if ($DisplayPopup -ne $true) {
        Write-DebugLine "$(Get-LineNumber)Show-SysmonAlert skipped because popup display is disabled."
        return $false
    }

    $msgText = "Sysmon Security Alert!`n`n$EventCount suspicious events found.`nClassification: $ClassificationSummary`n`nSaved as: Sysmon_Alert_$AlertTime.csv`n`nToAnalyze folder:`n$ToAnalyzeFolderName`n$ToAnalyzeFolder`n`nAutorun exported to: $SysmonReal`n`nCopy full ChatGPT analysis package to clipboard?"
...
# ===============================================================================
function New-SysmonClassificationResult {
    param(
        [Parameter(Mandatory=$true)]$Event,
    ...
    [PSCustomObject]@{
        ...
        ShouldRunAutoruns = $ShouldRunAutoruns
        GroupKey          = $GroupKey
    }
}

function Get-SysmonEventClassification {
    param(
        [Parameter(Mandatory=$true)]$Event
    )
    ...
    $EventProcessId = Get-SysmonDataValue -Data $Data -Names @("ProcessId")
    ...
    $ProcessGuid = Get-SysmonDataValue -Data $Data -Names @("ProcessGuid", "SourceProcessGuid")

    $GroupKey = "EventID=$EventId|Image=$Image|RuleName=$RuleName|Target=$TargetFilename$TargetObject|ProcessGuid=$ProcessGuid"

    if ($Message -match "__PSScriptPolicyTest|sdbinst\.exe|Autoruns64\.exe|Autorunsc64\.exe") {
        return New-SysmonClassificationResult -Event $Event -Class "Ignore" -Reason "Confirmed monitor/helper/tool self-noise by message pattern" -ShouldCollect $false -ShouldPopup $false -ShouldRunAutoruns $false -GroupKey $GroupKey
    }
    ...
    if (
        ...
        return New-SysmonClassificationResult -Event $Event -Class "Ignore" -Reason "Monitor copied its own ChatGPT package to clipboard" -ShouldCollect $false -ShouldPopup $false -ShouldRunAutoruns $false -GroupKey $GroupKey
    }
    ...
    if (
        ...
        $EventProcessId -eq [string]$PID -and
        $Image -match "(?i)\\powershell\.exe$" -and
        $TargetFilename -match "(?i)^Z:\\backup\\Data\\EventViewer\\SysmonReal\\(ToAnalyze|txt|Auto|ServiceContext|ConfigLoadContext|ClipboardEvidence)\\"
    ) {
        return New-SysmonClassificationResult -Event $Event -Class "Ignore" -Reason "Monitor-created evidence/output file from this process" -ShouldCollect $false -ShouldPopup $false -ShouldRunAutoruns $false -GroupKey $GroupKey
    }
    ...
    if (
        ...
        return New-SysmonClassificationResult -Event $Event -Class "KnownNoise" -Reason "PowerShell Add-Type / csc temporary compilation artifact" -ShouldCollect $false -ShouldPopup $false -ShouldRunAutoruns $false -GroupKey $GroupKey
    }
    ...
    if (
        ...
        return New-SysmonClassificationResult -Event $Event -Class "KnownNoise" -Reason "Known Windows compatibility-assistant registry inventory noise" -ShouldCollect $false -ShouldPopup $false -ShouldRunAutoruns $false -GroupKey $GroupKey
    }

    if (
        $EventId -eq 11 -and
        $TargetFilename -match "(?i)^Z:\\InsPro\\Tools\\Ps1\\ToastActions\\(Invoke-SysmonToastAction|Set-SysmonPopupState|Set-SysmonAlertState)\.ps1$"
    ) {
        return New-SysmonClassificationResult -Event $Event -Class "EvidenceOnly" -Reason "Exact trusted ToastActions helper file path; preserve evidence without popup" -ShouldCollect $true -ShouldPopup $false -ShouldRunAutoruns $false -GroupKey $GroupKey
    }

    if (
        $EventId -eq 11 -and
        $TargetFilename -match "(?i)^Z:\\InsPro\\Tools\\Logs\\Sysmon"
    ) {
        return New-SysmonClassificationResult -Event $Event -Class "EvidenceOnly" -Reason "Sysmon monitor/helper log file activity" -ShouldCollect $true -ShouldPopup $false -ShouldRunAutoruns $false -GroupKey $GroupKey
    }
    ...
    if (
        ...
        if (
            -not [string]::IsNullOrWhiteSpace($Configuration) -and
            $Configuration -ne "N/A" -and
            $Configuration -notmatch [regex]::Escape($ExpectedSysmonConfigPath)
        ) {
            return New-SysmonClassificationResult -Event $Event -Class "PopupHigh" -Reason "Sysmon configuration load from unexpected path: $Configuration" -ShouldCollect $true -ShouldPopup $true -ShouldRunAutoruns $true -GroupKey $GroupKey
        }

        return New-SysmonClassificationResult -Event $Event -Class "EvidenceOnly" -Reason "Expected Sysmon config-load evidence; no popup unless path/hash becomes unexpected" -ShouldCollect $true -ShouldPopup $false -ShouldRunAutoruns $false -GroupKey $GroupKey
    }
    ...
    if (
        ...
        return New-SysmonClassificationResult -Event $Event -Class "PopupHigh" -Reason "Suspicious process execution pattern" -ShouldCollect $true -ShouldPopup $true -ShouldRunAutoruns $true -GroupKey $GroupKey
    }

    if ($EventId -in @(8, 10)) {
        return New-SysmonClassificationResult -Event $Event -Class "PopupHigh" -Reason "High-risk process injection/access Sysmon event" -ShouldCollect $true -ShouldPopup $true -ShouldRunAutoruns $true -GroupKey $GroupKey
    }
    ...
    if (
        ...
        return New-SysmonClassificationResult -Event $Event -Class "PopupMedium" -Reason "Network connection involving watched administrative/mail/DNS/LDAP port" -ShouldCollect $true -ShouldPopup $true -ShouldRunAutoruns $false -GroupKey $GroupKey
    }

    if (
        $EventId -in @(12, 13) -and
        $TargetObject -match "(?i)\\(Run|RunOnce|Services|Winlogon|Image File Execution Options|TaskCache|StartupApproved|Shell Extensions|AppInit_DLLs)\\|\\Userinit$|\\Shell$|\\Debugger$"
    ) {
        return New-SysmonClassificationResult -Event $Event -Class "PopupMedium" -Reason "Registry change touches persistence/startup-sensitive location" -ShouldCollect $true -ShouldPopup $true -ShouldRunAutoruns $true -GroupKey $GroupKey
    }

    if ($EventId -in @(12, 13)) {
        return New-SysmonClassificationResult -Event $Event -Class "EvidenceOnly" -Reason "Registry event outside current persistence-sensitive list" -ShouldCollect $true -ShouldPopup $false -ShouldRunAutoruns $false -GroupKey $GroupKey
    }

    if (
        $EventId -in @(11, 15) -and
        $TargetFilename -match "(?i)\\Downloads\\[^\\]+\.ps1$" -and
        $Image -match "(?i)\\firefox\.exe$|\\firefoxdeveloperedition\.exe$|\\chrome\.exe$|\\opera\.exe$|\\librewolf\.exe$|\\msedge\.exe$"
    ) {
        return New-SysmonClassificationResult -Event $Event -Class "PopupMedium" -Reason "Browser-created PowerShell script in Downloads; do not globally ignore browser-created .ps1 files" -ShouldCollect $true -ShouldPopup $true -ShouldRunAutoruns $false -GroupKey $GroupKey
    }
    ...
    if (
        ...
        return New-SysmonClassificationResult -Event $Event -Class "PopupMedium" -Reason "Watched executable/script/file-location activity" -ShouldCollect $true -ShouldPopup $true -ShouldRunAutoruns $false -GroupKey $GroupKey
    }

    if ($EventId -eq 24) {
        return New-SysmonClassificationResult -Event $Event -Class "PopupMedium" -Reason "ClipboardChange from non-monitor process" -ShouldCollect $true -ShouldPopup $true -ShouldRunAutoruns $false -GroupKey $GroupKey
    }

    if ($EventId -in @(5, 7, 9, 17, 18)) {
        return New-SysmonClassificationResult -Event $Event -Class "PopupMedium" -Reason "High-interest Sysmon event type retained for review" -ShouldCollect $true -ShouldPopup $true -ShouldRunAutoruns $false -GroupKey $GroupKey
    }

    return New-SysmonClassificationResult -Event $Event -Class "Ignore" -Reason "Event does not match current collection/classification rules" -ShouldCollect $false -ShouldPopup $false -ShouldRunAutoruns $false -GroupKey $GroupKey
}
...
function Test-SysmonEventSuspicious {
    ...
    $Classification = Get-SysmonEventClassification -Event $Event
    return [bool]$Classification.ShouldCollect
}

function Get-SysmonBatchClassification {
    param(
        [Parameter(Mandatory=$true)]$Classifications
    )
    ...
    foreach ($Item in $Items) {
        $ClassName = [string]$Item.AlertClass
        if ([string]::IsNullOrWhiteSpace($ClassName)) {
            $ClassName = "Unknown"
        }

        if (-not $Counts.ContainsKey($ClassName)) {
            $Counts[$ClassName] = 0
        }
    ...
    $Priority = @("PopupHigh", "PopupMedium", "EvidenceOnly", "KnownNoise", "Ignore", "Unknown")
    $HighestClass = "Ignore"

    foreach ($ClassName in $Priority) {
        if ($Counts.ContainsKey($ClassName) -and $Counts[$ClassName] -gt 0) {
    ...
    $ShouldPopup = [bool](@($Items | Where-Object { $_.ShouldPopup -eq $true }).Count -gt 0)
    $ShouldRunAutoruns = [bool](@($Items | Where-Object { $_.ShouldRunAutoruns -eq $true }).Count -gt 0)
...
function Add-SysmonClassificationToReport {
    param(
        [Parameter(Mandatory=$true)]$Report,
        [Parameter(Mandatory=$true)]$Classifications
    )
    ...
    foreach ($Classification in @($Classifications)) {
        $Map[[string]$Classification.RecordID] = $Classification
    }
    ...
    foreach ($Row in @($Report)) {
        ...
        if ($Map.ContainsKey($Key)) {
            ...
            Add-Member -InputObject $Row -MemberType NoteProperty -Name "AlertClass" -Value $Classification.AlertClass -Force
            Add-Member -InputObject $Row -MemberType NoteProperty -Name "ClassificationReason" -Value $Classification.ClassificationReason -Force
            Add-Member -InputObject $Row -MemberType NoteProperty -Name "ShouldPopup" -Value $Classification.ShouldPopup -Force
            Add-Member -InputObject $Row -MemberType NoteProperty -Name "ShouldRunAutoruns" -Value $Classification.ShouldRunAutoruns -Force
            Add-Member -InputObject $Row -MemberType NoteProperty -Name "ClassificationGroupKey" -Value $Classification.GroupKey -Force
        }
        else {
            Add-Member -InputObject $Row -MemberType NoteProperty -Name "AlertClass" -Value "Unknown" -Force
            Add-Member -InputObject $Row -MemberType NoteProperty -Name "ClassificationReason" -Value "No classification found for row RecordID" -Force
            Add-Member -InputObject $Row -MemberType NoteProperty -Name "ShouldPopup" -Value $false -Force
            Add-Member -InputObject $Row -MemberType NoteProperty -Name "ShouldRunAutoruns" -Value $false -Force
            Add-Member -InputObject $Row -MemberType NoteProperty -Name "ClassificationGroupKey" -Value "N/A" -Force
        }
...
$CheckFile = CheckPopupFileExist -File $AlertPath
if ($CheckFile -eq $true) {
	Write-DebugLine "$(Get-LineNumber)Checking Popup File-$AlertPath." 
	Remove-Item -Path $AlertPath -ErrorAction SilentlyContinue -Force
...
while ($true) {
	...
	$CheckFile = CheckPopupFileExist -File $AlertPath
    if ($CheckFile -eq $true) {
        Write-DebugLine "$(Get-LineNumber)File Path = $AlertPath exists."
        
        # Read the file content cleanly and safely
        $FileContent = (Get-Content -Path $AlertPath -ErrorAction SilentlyContinue)
        ...
        if ($null -ne $FileContent) {
            ...
            if ($FileContent.ToString().Trim() -eq "UNSET") {
                $Set = $false
            }
			...
			if ($Result -eq $true) {
				...
				$DisplayPopup = $true
                Write-DebugLine "$(Get-LineNumber)File is missing SET content Diaply Popup set to $DisplayPopup"
            }
            Write-DebugLine "$(Get-LineNumber)Removing file....$AlertPath" 
            Remove-Item -Path $AlertPath -ErrorAction SilentlyContinue -Force
       }
    }else{
		 Write-DebugLine "$(Get-LineNumber)File Path = $AlertPath not exists."
	}
        ...
        if ($Suspicious -and $Suspicious.Count -gt 0) {
            $EventClassifications = @(
                $Suspicious | ForEach-Object {
                    Get-SysmonEventClassification -Event $_
                }
            )
            $BatchClassification = Get-SysmonBatchClassification -Classifications $EventClassifications
			...
			$NextCheckTime = (Get-Date).AddSeconds($MonitorIntervalSeconds)
            $ShouldInterruptUser = [bool](($DisplayPopup -eq $true) -and ($BatchClassification.ShouldPopup -eq $true))
            $ShouldRunAutorunsForBatch = [bool]$BatchClassification.ShouldRunAutoruns

			if($ShouldInterruptUser -eq $true){
				Write-DebugLine "$(Get-LineNumber)`n[$CheckTime] ALERT: $($Suspicious.Count) new suspicious events detected, Classification=$($BatchClassification.HighestClass), Popup=YES, Autoruns=$ShouldRunAutorunsForBatch, Summary=[$($BatchClassification.Summary)] - Next check: $NextCheckTime" 
			} else{
				Write-DebugLine "$(Get-LineNumber)`n[$CheckTime] ALERT: $($Suspicious.Count) new classified events detected, Classification=$($BatchClassification.HighestClass), Popup=NO, Autoruns=$ShouldRunAutorunsForBatch, Summary=[$($BatchClassification.Summary)] - Next check: $NextCheckTime"
			}
            ...
            $Report = Add-SysmonClassificationToReport -Report $Report -Classifications $EventClassifications
            ...
            if ($ShouldInterruptUser -eq $true) {
				Write-DebugLine "$(Get-LineNumber)Sending alert toast before heavy evidence. ToAnalyzeFolderName=$ToastToAnalyzeFolderName Classification=$($BatchClassification.HighestClass) Summary=[$($BatchClassification.Summary)]"
				Send-SysmonToast -Message ("Sysmon Security Alert. Class=" + $BatchClassification.HighestClass + " -ToAnalyze folder: " + $ToastToAnalyzeFolderName) -Title "Warning"
                ...
                Write-DebugLine "$(Get-LineNumber)Popup/toast skipped by classification. Classification=$($BatchClassification.HighestClass) Summary=[$($BatchClassification.Summary)] AlertStamp=$AlertStamp"
            }

			if ((Test-Path -Path $Autorun) -and ($ShouldRunAutorunsForBatch -eq $true)) {
				try {
					Write-DebugLine "$(Get-LineNumber)Autoruns dump started after classification gate for AlertStamp=$AlertStamp Classification=$($BatchClassification.HighestClass)"
					...
					Write-DebugLine "$(Get-LineNumber)Autoruns dump completed after classification gate for AlertStamp=$AlertStamp"
				}
				catch {
					Write-DebugLine "$(Get-LineNumber)Autoruns dump failed after classification gate: $($_.Exception.Message)"
				}
			}
            else {
                Write-DebugLine "$(Get-LineNumber)Autoruns dump skipped by classification gate or missing executable. ShouldRunAutoruns=$ShouldRunAutorunsForBatch AutorunPath=$Autorun Exists=$(Test-Path -Path $Autorun)"
            }
			...
			$NextCheckTime = (Get-Date).AddSeconds($MonitorIntervalSeconds)
			if ( $DisplayPopup -eq $true  ) {
				$Line = "Clean - No new suspicious activity with Popup - Next check: $NextCheckTime"
				#Send-SysmonToast -Message "Sysmon Security Alert with Popup - No new suspicious activity." -Title "Message"
			} else{
				$Line = "Clean - No new suspicious activity without popup - Next check: $NextCheckTime"

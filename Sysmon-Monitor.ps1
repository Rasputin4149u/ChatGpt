...
function Send-SysmonToast {
    ...
    if ($DisplayPopup -eq $true ){
		$ToastXmlText = @"
<toast launch='powershell://action?type=open'>
		<visual>
...
function Test-SysmonEventSuspicious {
	...
	# Suppress Sysmon-Monitor evidence-pack file creation self-noise.
	if (
		$EventId -eq 11 -and
		$EventProcessId -eq [string]$PID -and
		$Image -match "(?i)\\powershell\.exe$" -and
		$TargetFilename -match "(?i)^Z:\\backup\\Data\\EventViewer\\SysmonReal\\ToAnalyze\\"
	) {
		return $false
	}
...
while ($true) {
    ...
    try {
        ...
        if ($Suspicious -and $Suspicious.Count -gt 0) {
			...
			if($DisplayPopup -eq $true){
				Write-DebugLine "$(Get-LineNumber)`n[$CheckTime] ALERT: $($Suspicious.Count) new suspicious events detected, Displaying Popup! - Next check: $NextCheckTime" 
			} else{
				Write-DebugLine "$(Get-LineNumber)`n[$CheckTime] ALERT: $($Suspicious.Count) new suspicious events detected, Without Popup! - Next check: $NextCheckTime"
			}
